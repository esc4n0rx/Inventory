...
class FinalizarInventarioDialog(QDialog):
    ...
    def _carregar_detalhes_cds(self, dados):
        ...
        self._adicionar_linha_origem(self.tabela_cd_sp, "CD SP", 
                                  {tipo: totais.get(f'cd_sp_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        # Linha para Trânsito SP
        self._adicionar_linha_origem(self.tabela_cd_sp, "Trânsito SP", 
                                  {tipo: totais.get(f'transito_sp_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        # Linha para Fornecedor SP
        self._adicionar_linha_origem(self.tabela_cd_sp, "Fornecedor SP", 
                                  {tipo: (totais.get(f'fornecedor_{tipo}', 0) or 0) / 3 for tipo in tipos_caixa})
        ...
        self._adicionar_linha_origem(self.tabela_cd_es, "CD ES", 
                                  {tipo: totais.get(f'cd_es_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        # Linha para Trânsito ES
        self._adicionar_linha_origem(self.tabela_cd_es, "Trânsito ES", 
                                  {tipo: totais.get(f'transito_es_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        # Linha para Fornecedor ES
        self._adicionar_linha_origem(self.tabela_cd_es, "Fornecedor ES", 
                                  {tipo: (totais.get(f'fornecedor_{tipo}', 0) or 0) / 3 for tipo in tipos_caixa})
        ...
        # Linha para CD RJ (Físico)
        self._adicionar_linha_origem(self.tabela_cd_rj, "CD RJ", 
                                  {tipo: totais.get(f'cd_rj_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        # Linha para Trânsito RJ
        self._adicionar_linha_origem(self.tabela_cd_rj, "Trânsito RJ", 
                                  {tipo: totais.get(f'transito_rj_{tipo}', 0) or 0 for tipo in tipos_caixa})
        
        
        # Linha para Fornecedor RJ
        self._adicionar_linha_origem(self.tabela_cd_rj, "Fornecedor RJ", 
                                  {tipo: (totais.get(f'fornecedor_{tipo}', 0) or 0) / 3 for tipo in tipos_caixa})
    ...
    def _carregar_detalhes_lojas(self, dados):
        ...
        # Obter conexão para consulta
        conn = self.inventario_service.db_manager.get_connection()
        cursor = conn.cursor()
        
        # Consultar os dados de todas as lojas no inventário atual
        cursor.execute('''
